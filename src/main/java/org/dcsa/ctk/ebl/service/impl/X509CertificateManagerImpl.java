...
public class X509CertificateManagerImpl implements X509CertificateManager {
    ...
    public X509Certificate removeSignature(X509Certificate signedCertificate) throws NoSuchAlgorithmException, OperatorCreationException, CertificateException {
        ...
        X500Name x500name = new JcaX509CertificateHolder(signedCertificate).getSubject();
        RDN cn = x500name.getRDNs(BCStyle.CN)[0];

        String clientSubject = rootSubject.replace("Root CA", cn.toString());
    ...
    public String makeClientCertificate(CertificateInfo certificateInfo, PublicKey publicKey, CertificateTrust certificateTrust) throws Exception {
        ...
        CertificateUtil.getCertificateFile(certificateName, this);
        return certificateTrust.name()+" certificate is created. Pls call http://localhost:9001/ebl/getCertificate by browser.";
    }
...
/*        GeneralName generalName = new GeneralName(GeneralName.uniformResourceIdentifier, new DERIA5String(crlUri));
    ...
    public  boolean isClientCertificateInitialized(){
        if( clientCertificate == null && certificateName == null ){
            return false;
        }else{
            return true;
        }
    ...
    public String getCertificateStr(){
        ...
        sw.append("Pls call GET /ebl/getCertificate by browser.");
        return sw.toString();
    }
