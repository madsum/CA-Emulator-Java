...
public class CertificateController {
    ...
    public String  makeCertificate(@PathVariable String sign,  @RequestBody CertificateInfo certificateInfo) throws Exception {
    ...
    @PutMapping(path = SIGN_CERTIFICATE_FILE)
    ...
    public String revokeSignature(@RequestParam("file") MultipartFile file) throws Exception {
        X509Certificate  certificate = CertificateUtil.makeCertificateFromMultipartFile(file);
        X509Certificate x509Certificate =  x509CertificateManager.removeSignature(certificate);
        x509CertificateManager.setClientCertificate(x509Certificate);
        return "certificate signature is revoked. Pls call http://localhost:9001/ebl/getCertificate by browser." ;
    }
}
